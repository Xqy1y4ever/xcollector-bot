...
刻意不做的事：
  - **不在这里下载附件**。归一化只负责把 URL 挑出来；真正的"下载字节 → 上传给后端"
    发生在 pipeline/runner.py 里，而且是在**写前日志之后**（原文先落库，附件事后补齐）。
    这条分工线是"原文是唯一不可再生的资产"这条原则的直接后果。
  - 不做任何白名单判断（群/发送者白名单在 runner 里，用 bot 自己的配置）。
"""
