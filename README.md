...
`rule` 是**故意收得比较紧**的确定性抽取器：必须命中"通知类词"（通知/请/截止/报名/提交/
体检/开会/…）才可能建条，**只有时间词不算** —— 「@张三 明天」「我下周三可能去不了」
这类带日期的闲聊不会变成任务。它不花钱、结果可复现，适合本地联调；判得不够准，也可能
漏掉用词不典型的真通知。

`both` / `llm` 下 **是不是通知由模型判断**，而**相对时间的日期换算由代码兜底**：

- 提示词里直接给一张按消息发送时间算好的日历（本周/下周/再下周、今天/明天/后天、月底），
  模型查表即可 —— 「下周三」「这周天」「月底前」不再靠它自己做日期运算；
- 模型没给出 `due_at` 时，用 `timeparse` 在同一段原文里算出的时间补上，`extractor`
  记成 `llm+rule`、候选列表里写明来源（前端详情看得到），把握度不超过 0.8；
- 模型判"不是通知"时会写 `reason` 进日志；**原文照常入库**（`state=noise`），
  所以"少了哪条、为什么少"查得到。
